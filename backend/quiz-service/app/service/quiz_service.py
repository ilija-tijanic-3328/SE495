...
from app.service import participation_client, quiz_config_service
...
def validate_status(quiz: Quiz):
    ...
    if now > quiz.end_time.replace(tzinfo=timezone.utc):
...
def validate_existing(quiz: Quiz, title: str, time_allowed: int, start_time: datetime,
    ...
    now = datetime.now(timezone.utc)
    if now > quiz.start_time.replace(tzinfo=timezone.utc) != start_time:
